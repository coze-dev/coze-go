...
func TestConversationMessage(t *testing.T) {
	...
	t.Run("feedback create success", func(t *testing.T) {
		conversationID := randomString(10)
		...
		resp, err := messages.Feedback.Create(context.Background(), &CreateConversationMessageFeedbackReq{
			ConversationID: conversationID,
			MessageID:      messageID,
	...
	t.Run("feedback create minimal", func(t *testing.T) {
		conversationID := randomString(10)
		messageID := randomString(10)
		messages := newConversationMessage(newCoreWithTransport(newMockTransport(func(req *http.Request) (*http.Response, error) {
			as.Equal(http.MethodPost, req.Method)
		...
		resp, err := messages.Feedback.Create(context.Background(), &CreateConversationMessageFeedbackReq{
			ConversationID: conversationID,
			MessageID:      messageID,
			FeedbackType:   FeedbackTypeUnlike,
		})
		as.Nil(err)
		as.NotNil(resp)
		as.Equal("test_log_id", resp.LogID())
	...
	t.Run("feedback delete success", func(t *testing.T) {
		conversationID := randomString(10)
		messageID := randomString(10)
		messages := newConversationMessage(newCoreWithTransport(newMockTransport(func(req *http.Request) (*http.Response, error) {
		...
		resp, err := messages.Feedback.Delete(context.Background(), &DeleteConversationMessageFeedbackReq{
			ConversationID: conversationID,
			MessageID:      messageID,
		})
		as.Nil(err)
